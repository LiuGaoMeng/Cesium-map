    ...
    methods: {
        ...
        initObject() {
            const box = this.viewer.entities.add({
                // id: 'ellipse', // 对象的唯一标识符。如果未提供，则将生成GUID。
                name: 'ellipse', // 要显示给用户的可读名称。它不必是唯一的。
                position: Cesium.Cartesian3.fromDegrees(-107.0, 40, 10000),
                ellipse: {
                    semiMajorAxis: 1000, // 半长轴
                    semiMinorAxis: 1000, // 半短轴
                    height: 1000,
                    extrudedHeight: 500, // 椭圆的挤压面相对于椭球面的高度。
                    extrudedHeightReference: Cesium.HeightReference.NONE, // 用于指定extrudedHeight相对于什么。类比heightReference
                    material: '/image/billboard.jpg', // 填充颜色
                    show: true, // 是否展示
                    rotation: Cesium.Math.toRadians(90), // 北方逆时针旋转。旋转90度
                    stRotation: Cesium.Math.toRadians(90), // 纹理旋转，一般同rotation一致 旋转90度
                    // granularity: ,// 定椭圆上各点之间的角距离。即对圆平分，越多圆越平滑
                    // numberOfVerticalLines:,// 沿轮廓的周长绘制的垂直线的数量。
            ...
            this.viewer.entities.add({
                name: 'ellipse', // 要显示给用户的可读名称。它不必是唯一的。
                position: Cesium.Cartesian3.fromDegrees(-107.0, 40.1, 10000),
                ellipse: {
                    semiMajorAxis: 8000, // 半长轴
                    semiMinorAxis: 4000, // 半短轴
                    // height: 1000,
                    // extrudedHeight: 0,
